...
\end{align}
Let's define the cost function of the first layer $\arg[C, 1]$ as:
\begin{align}
    \pd[{\arg[a_i, 1]}]C^{(2)} &=  \avgsum[i, n]2 \cdot (a_i^{(2)} - z_i) \cdot a_i^{(1)}(1 - a_i^{(1)}) \cdot \arg[w, 2]    \\
    e_i  &= \arg[a_i, i] - (\arg[a_i, i] - \pd[{\arg[a_i, 1]}]C^{(2)})\\
    \arg[C, 1] &= \avgsum[i, n] (\arg[a_i, 1] - e_i) \\
\end{align}
Let's compute the derivitive $C^{(1)}$$'$ of our function\\
\end{document}
